...
I profiled actual inference using DistilGPT-2 to measure prefill and decode times separately.
...
Short output (20 tokens):   339ms  (prefill: 52ms, decode: 287ms)
Medium output (50 tokens):   793ms  (prefill: 52ms, decode: 741ms)
Long output (100 tokens):  1,561ms  (prefill: 52ms, decode: 1,509ms)
```

**Key observations:**
1. Prefill time is constant (~52ms) regardless of output length
2. Decode time scales linearly—each token adds ~15ms
3. The 4.6× speedup comes entirely from fewer decode iterations

This directly explains your 3.2× improvement (10,652ms → 3,266ms). Your LoRA adapter learned to generate ~3× fewer tokens, which reduced decode iterations proportionally.
...
**Why fine-tuning beats prompt engineering for latency:** Prompt engineering adds tokens to your input, which increases prefill cost slightly. More importantly, it relies on in-context learning to steer the model's generation, which is less reliable than weight updates. A fine-tuned model has internalized the task-specific output distribution through training on examples of concise, task-specific outputs — it "knows" to stop generating after the relevant answer, rather than continuing with verbose explanations or examples. The training data taught the model when to emit the EOS token, which is more reliable than relying on in-context learning from a system prompt. This is why your LoRA adapter outperforms prompt engineering: the behavior is encoded in the weights, not inferred from context.
...
- **Tool/pattern used:** Real inference profiling ([demo_decode_latency.py](./demo_decode_latency.py)) using transformers library to measure prefill and decode times separately across different output lengths.
